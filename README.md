...
This implementation has been in production use on the Viacoin mainnet since early 2018, with the launch of the [Blockstream Store][blockstream-store-blog].
...
c-lightning only works on Linux and Mac OS, and requires a locally (or remotely) running `viacoind` (version 0.16 or above) that is fully caught up with the network you're testing on.
Pruning (`prune=n` option in `viacoin.conf`) is partially supported, see [here](#pruning) for more details.
...
sudo add-apt-repository -u ppa:viacoin/viacoin
sudo add-apt-repository -u ppa:lightningnetwork/ppa
sudo apt-get install viacoind lightningd
```

### Starting `lightningd`

If you want to experiment with `lightningd`, there's a script to set
up a `viacoind` regtest test network of two local lightning nodes,
...
To test with real viacoin,  you will need to have a local `viacoind` node running:

```bash
viacoind -daemon
```

Wait until `viacoind` has synchronized with the network.

Make sure that you do not have `walletbroadcast=0` in your `~/.viacoin/viacoin.conf`, or you may run into trouble.
...
```bash
lightningd --network=viacoin --log-level=debug
```
...
* [newaddr](doc/lightning-newaddr.7.md): get a viacoin address to deposit funds into your lightning node.
...
viacoin-cli sendtoaddress <address> <amount_in_viacoins>
```
...
(eg. `~/.lightning/config` or `~/.lightning/viacoin/config`).  See `man -l doc/lightningd-config.5`.

## Further information

### Pruning

c-lightning requires JSON-RPC access to a fully synchronized `viacoind` in order to synchronize with the Viacoin network.
Access to ZeroMQ is not required and `viacoind` does not need to be run with `txindex` like other implementations.
The lightning daemon will poll `viacoind` for new blocks that it hasn't processed yet, thus synchronizing itself with `viacoind`.
If `viacoind` prunes a block that c-lightning has not processed yet, e.g., c-lightning was not running for a prolonged period, then `viacoind` will not be able to serve the missing blocks, hence c-lightning will not be able to synchronize anymore and will be stuck.
In order to avoid this situation you should be monitoring the gap between c-lightning's blockheight using `lightning-cli getinfo` and `viacoind`'s blockheight using `viacoin-cli getblockchaininfo`.
...
If you encrypt your `hsm_secret`, you will have to pass the `--encrypted-hsm` startup option to `lightningd`. Once your `hsm_secret` is encrypted, you __will not__ be able to access your funds without your password, so please beware with your password management. Also beware of not feeling too safe with an encrypted `hsm_secret`: unlike for `viacoind` where the wallet encryption can restrict the usage of some RPC command, `lightningd` always need to access keys from the wallet which is thus __not locked__ (yet), even with an encrypted BIP32 master seed.
